export class LinuxServerAuditorListPage {
        ...
        return `
            <div style="flex: 1; overflow: auto; min-height: 0; border-radius: 6px;">
                <table class="audit-table-v2 compact-mode" style="font-size: 0.75rem; width: 100%; border-collapse: collapse; border-spacing: 0; table-layout: fixed;">
                    <thead>
                        <tr>
                            <th onclick="linuxServerAuditorListInstance.handleSort('name')" style="width: 35%; padding: 0.75rem 0.5rem; cursor: pointer; user-select: none; position: sticky; top: 0; background: #0f172a; z-index: 10; border-bottom: 2px solid rgba(255, 255, 255, 0.1); text-align: left;">
                                ${this.t('name')} ${this.sortField === 'name' ? `<i class="fas fa-sort-${this.sortOrder === 'asc' ? 'up' : 'down'}" style="margin-left: 0.25rem;"></i>` : ''}
                            </th>
                            <th onclick="linuxServerAuditorListInstance.handleSort('serverName')" style="width: 30%; padding: 0.75rem 0.5rem; cursor: pointer; user-select: none; position: sticky; top: 0; background: #0f172a; z-index: 10; border-bottom: 2px solid rgba(255, 255, 255, 0.1); text-align: left;">
                            ...
                            <th onclick="linuxServerAuditorListInstance.handleSort('createdAt')" style="width: 12%; padding: 0.75rem 0.5rem; cursor: pointer; user-select: none; position: sticky; top: 0; background: #0f172a; z-index: 10; border-bottom: 2px solid rgba(255, 255, 255, 0.1); text-align: left;">
                    ...
                    <tbody>
                        ${filteredReports.map(report => `
                            <tr onclick="linuxServerAuditorListInstance.viewReport(${report.id})" style="cursor: pointer;">
                                ...
                                <td style="padding: 0.35rem 0.5rem; text-align: right; white-space: nowrap;">
                                    <button class="premium-action-btn" style="display: inline-flex; width: 24px; height: 24px; padding: 0; justify-content: center; font-size: 0.75rem;" onclick="event.stopPropagation(); linuxServerAuditorListInstance.triggerImport(${report.id})" title="${this.t('import')}">
                                        <i class="fas fa-upload"></i>
                                    </button>
                                    <button class="premium-action-btn" style="display: inline-flex; width: 24px; height: 24px; padding: 0; justify-content: center; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); font-size: 0.75rem;" onclick="event.stopPropagation(); linuxServerAuditorListInstance.deleteReport(${report.id})" title="${this.t('delete')}">
    ...
    async render() {
        return `
            <div class="page-container-full" style="height: calc(100vh - 120px); overflow: hidden; display: flex; flex-direction: column;">
                <input type="file" id="linux-server-list-file-input" style="display: none" onchange="linuxServerAuditorListInstance.handleFileSelect(event)">
                <div style="padding: 1.5rem; display: flex; flex-direction: column; height: 100%; box-sizing: border-box;">
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0; flex-shrink: 0; gap: 1rem;">
                        <div style="display: flex; align-items: center; gap: 0.75rem; flex: 1;">
                            <div style="position: relative; flex: 0 1 300px;">
                                ...
                                <input type="text" 
                                       ...
                                       oninput="linuxServerAuditorListInstance.handleSearch(this.value)"
                        ...
                        <div style="display: flex; gap: 0.5rem; flex-shrink: 0;">
                            <button class="premium-action-btn" onclick="linuxServerAuditorListInstance.loadReports()" style="width: 32px; height: 32px; padding: 0; display: flex; justify-content: center; align-items: center;" title="Refresh">
                                <i class="fas fa-sync-alt ${this.loadingReports ? 'fa-spin' : ''}" style="font-size: 0.75rem;"></i>
                            </button>
                            
                            <button class="premium-action-btn primary" onclick="linuxServerAuditorListInstance.showCreateReportModal()" style="padding: 0 0.6rem; font-size: 0.75rem; height: 32px;">
    ...
    triggerImport(reportId) {
        ...
        const fileInput = document.getElementById('linux-server-list-file-input');
        if (fileInput) {
            fileInput.click();
        }
    }
